...
const personalityQuizAPIBaseUrl =
	"https://t4j3jcpjbg.execute-api.ap-southeast-2.amazonaws.com/prod";
...
class PersonalityService {
	...
	async analyzePersonality(
		params: AnalyzePersonalityParams,
		...
		try {
			...
			throw new Error(
				`Failed to submit personality analysis: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
	...
	async getJobStatus(
		...
		const statusUrl = personalityAnalysisJobStatusAPI.replace(
			"{sessionId}",
			jobId,
		);
		...
		try {
			...
			if (retryCount < MAX_RETRY_ATTEMPTS) {
				console.warn(
					`Status check failed, retrying... (attempt ${retryCount + 1}/${MAX_RETRY_ATTEMPTS + 1})`,
				);
	...
	async pollForCompletion(
		...
		while (true) {
			...
			if (currentTime - startTime > MAX_POLLING_TIMEOUT) {
				throw new Error(
					`Job polling timeout after ${MAX_POLLING_TIMEOUT / 1000} seconds`,
				);
			}
				...
				switch (statusResponse.status) {
					case "success":
						if (statusResponse.traits && statusResponse.summary) {
							...
							console.log(
								"⚠️  Success status but missing traits/summary. Creating mock result for testing...",
							);
							// Temporary mock result for testing when API doesn't return expected structure
							const mockResult: JobStatusResponse = {
								...statusResponse,
								personalityId:
									"personality_" + Math.random().toString(36).substr(2, 9),
								traits: [
									{
										traitId: "trait_1",
										...
										confidence: 0.95,
										description:
											"This pet shows exceptional loyalty and forms strong bonds with their family.",
									},
									{
										traitId: "trait_2",
										name: "Playful Spirit",
										...
										confidence: 0.88,
										description:
											"An energetic and playful personality that loves interactive games and activities.",
									},
								],
								confidence: 0.9,
								sessionId: jobId,
								summary:
									...
									"Your pet has a wonderful, well-balanced personality with strong loyalty traits and a playful nature. They form deep bonds with family members and bring joy through their energetic and loving spirit.",
							};
					...
					case "failed":
						throw new Error(
							`Job failed: ${statusResponse.error || statusResponse.message || "Unknown error"}`,
						);
					...
					case "processing":
						// Continue polling
						console.log(
							`Job ${jobId} still ${statusResponse.status}, waiting ${POLLING_INTERVAL / 1000}s...`,
						);
	...
	buildAnalysisRequestBody(
		...
		const quizAnswers: QuizAnswer[] = responses.map((response) => {
			...
			if (!question) {
				throw new Error(
					`Question not found for questionId: ${response.questionId}`,
				);
			}
