	...
	const checkJobStatus = async (jobId: string) => {
		...
		try {
			...
			if (status.status === "success") {
				alert(
					`${pendingPets.find((p) => p.jobId === jobId)?.petName}'s story is ready! 🎉`,
				);
			}
		} catch (error) {
		...
		<div className="mb-8">
			...
			<div className="space-y-4">
				{pendingPets.map((pet) => (
					<div
						key={pet.jobId}
						className="bg-white border-l-4 border-blue-500 rounded-lg shadow p-4"
					>
						<div className="flex items-center justify-between">
							...
							<div className="flex items-center space-x-4">
								<div className="text-right">
									<span
										className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(pet.status)}`}
									>
										{pet.status}
									</span>
									{pet.estimatedWaitTime && (
										<p className="text-xs text-gray-500 mt-1">
											Est: {pet.estimatedWaitTime}
										</p>
									)}
								</div>
								<button
