...
export default function PetDetailPage() {
	...
	const checkFinalStatus = async () => {
		try {
			...
			if (status.status === "success") {
				console.log(
					"✅ Job completed successfully, setting personality result:",
					status,
				);
	...
	return (
		<div className="px-4 py-6 sm:px-0">
					...
					<div className="flex items-center space-x-4">
						<div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center">
							<span className="text-2xl">
								{getPetEmoji(pendingPet?.petType || "dog")}
							</span>
						</div>
						<div>
			...
			{jobStatus && (
				<div
					className={`border rounded-lg p-6 mb-6 ${getStatusColor(jobStatus.status)}`}
				>
					<div className="flex items-center justify-between">
						<div>
							...
							{jobStatus.message && (
								<p className="text-sm mb-2">{jobStatus.message}</p>
							)}
						...
						<div className="text-right">
							...
							{pendingPet && !jobStatus.createdAt && (
								<p className="text-xs">
									Created: {new Date(pendingPet.createdAt).toLocaleString()}
								</p>
							)}
						</div>
					</div>
				...
				<div className="bg-white rounded-lg shadow p-8 text-center">
					<div className="mb-6">
						...
						<p className="text-gray-600 mb-4">
							We're analyzing their personality and creating a unique profile
							for {pendingPet?.petName}.
						</p>
