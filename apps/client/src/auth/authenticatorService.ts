...
export class AuthenticatorService {
   ...
   */
  async createAuthenticator(authenticatorData: CreateAuthenticatorRequest): Promise<Authenticator> {
  ...
  async getAuthenticator(credentialID: string): Promise<Authenticator | null> {
    try {
      return await this.request<Authenticator>(`/api/authenticators/${encodeURIComponent(credentialID)}`, {
        method: 'GET',
      });
    } catch (error) {
   ...
   */
  async updateAuthenticatorCounter(credentialID: string, counter: number): Promise<Authenticator> {
    return this.request<Authenticator>(`/api/authenticators/${encodeURIComponent(credentialID)}/counter`, {
      method: 'PUT',
  ...
  async deleteAuthenticator(credentialID: string): Promise<void> {
    await this.request<void>(`/api/authenticators/${encodeURIComponent(credentialID)}`, {
      method: 'DELETE',
    });
  }
   ...
   */
  async getAllAuthenticators(query: GetAuthenticatorsQuery = {}): Promise<Authenticator[]> {
    const searchParams = new URLSearchParams();
    
    if (query.userId) searchParams.set('userId', query.userId);
    if (query.credentialID) searchParams.set('credentialID', query.credentialID);
