...
export class VerificationTokenService {
   ...
   */
  async createVerificationToken(tokenData: CreateVerificationTokenRequest): Promise<VerificationToken> {
  ...
  async useVerificationToken(identifier: string, token: string): Promise<VerificationToken | null> {
    try {
      return await this.request<VerificationToken>(`/api/verification-tokens/${encodeURIComponent(identifier)}/${encodeURIComponent(token)}/use`, {
        method: 'DELETE',
      });
    } catch (error) {
  ...
  async getVerificationToken(identifier: string, token: string): Promise<VerificationToken | null> {
    try {
      return await this.request<VerificationToken>(`/api/verification-tokens/${encodeURIComponent(identifier)}/${encodeURIComponent(token)}`, {
        method: 'GET',
      });
    } catch (error) {
      if (error instanceof ServiceError && error.statusCode === 404) {
  ...
  async cleanupExpiredTokens(): Promise<{ message: string; deletedCount: number }> {
    return this.request<{ message: string; deletedCount: number }>('/api/verification-tokens/cleanup', {
      method: 'DELETE',
    });
  }
}
