...
export function DatabaseAdapter(): Adapter {
  return {
    ...
    unlinkAccount: async ({ provider, providerAccountId }) => {
      try {
        const account = await accountService.unlinkAccount(provider, providerAccountId);
        return account as unknown as AdapterAccount;
      } catch {
    ...
    getAccount: async (providerAccountId, provider) => {
      try {
        const account = await accountService.getAccount(provider, providerAccountId);
    ...
    getUserByAccount: async ({ provider, providerAccountId }) => {
      try {
        const user = await accountService.getUserByAccount(provider, providerAccountId);
    ...
    updateSession: async (data) => {
      try {
        const session = await sessionService.updateSession(data.sessionToken, data);
        return session as AdapterSession;
      } catch {
    ...
    createVerificationToken: async (data) => {
      try {
        const token = await verificationTokenService.createVerificationToken(data as any);
    ...
    useVerificationToken: async ({ identifier, token }) => {
      try {
        const verificationToken = await verificationTokenService.useVerificationToken(identifier, token);
    ...
    createAuthenticator: async (data) => {
      try {
        const authenticator = await authenticatorService.createAuthenticator(data as any);
    ...
    getAuthenticator: async (credentialID) => {
      try {
        const authenticator = await authenticatorService.getAuthenticator(credentialID);
        return authenticator as any;
      } catch {
        return null;
      }
    ...
    listAuthenticatorsByUserId: async (userId) => {
      try {
        const authenticators = await authenticatorService.getAuthenticatorsByUserId(userId);
    ...
    updateAuthenticatorCounter: async (credentialID, counter) => {
      try {
        const authenticator = await authenticatorService.updateAuthenticatorCounter(credentialID, counter);
        return authenticator as any;
      } catch {
