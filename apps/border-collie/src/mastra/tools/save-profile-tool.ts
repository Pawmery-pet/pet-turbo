...
import { PetClient, CreatePetProfileSchema, PetProfileSchema } from "@repo/pet-client";
import { z } from "zod";
...
export const savePersonalityProfileTool = createTool({
	...
	execute: async (context) => {
		...
		const client = new PetClient(process.env.PET_SERVICE_URL ?? "http://localhost:3020");
		return client.createProfile(petId, { userId, traits, narrative });
	},
});
